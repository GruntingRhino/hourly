...
router.get("/:id/students", authenticate, requireRole("SCHOOL_ADMIN", "TEACHER"), async (req: Request, res: Response) => {
  try {
    ...
    const result = students.map((student) => ({
      ...
      remainingHours: progressMap.get(student.id)?.remainingHours ?? school.requiredHours,
      percentComplete: progressMap.get(student.id)?.percentComplete ?? 0,
      status: progressMap.get(student.id)?.status ?? "ON_TRACK",
      riskLevel: progressMap.get(student.id)?.riskLevel ?? "LOW",
