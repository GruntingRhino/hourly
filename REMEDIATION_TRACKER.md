...
| Issue | Status | Evidence |
|---|---|---|
| `IS_PROD_LIKE` duplicated independently in 7 files; `routes/schools.ts`'s copy tested only `NODE_ENV`/`VERCEL_ENV` and missed `APP_ENV`, so an `APP_ENV`-only production config would leak a newly created teacher's temp password in the create-staff API response | ✅ | Centralized into `server/src/lib/isProdLike.ts` (deliberately NOT `env.ts`, which runs `process.exit(1)` at import time and would break any lightweight importer). All 7 files now import it. Test: `isProdLike.test.ts`. Commit `273f531`. **Follow-up, found later this session**: a repo-wide sweep for the same anti-pattern found 2 more files the original "7 files" centralization missed — `routes/internal.ts` (gated whether 5 internal cron endpoints require `CRON_SECRET` to be configured; a defense-in-depth policy gap, not an auth bypass, since `hasValidSchedulerAuth()` independently required valid scheduler auth regardless) and, **more seriously, `lib/fieldEncryption.ts`** (gated whether a missing/malformed `FIELD_ENCRYPTION_KEY` throws at startup or silently falls through to storing PII — specifically phone numbers, per `routes/auth.ts` — in **plaintext** with only a console warning; an `APP_ENV`-only production deployment without the key configured would have silently stored phone numbers unencrypted). Both fixed (commit `c356372`) by importing the canonical module. Added a repo-wide architecture test (`isProdLikeNoDuplicateDefinitions.test.ts`) scanning every route/lib/service file for a local `function isProdLike` definition, so this exact class of drift can't silently reoccur in any future file — the narrower existing tests only covered the specific sites already known at the time they were written. **Second follow-up, found while scoping §15** (below): that architecture test's regex only matched a *function named* `isProdLike`, so 4 more instances using a different local name/shape dodged it entirely — found by grepping for the raw `NODE_ENV`/`VERCEL_ENV === "production"` comparison pattern itself instead of a name. Most severe: `routes/googleAuth.ts`'s local `IS_PRODUCTION` const (missing `APP_ENV`) left the fully-unauthenticated `POST /dev-signin` route (mints a real session for any email with zero Google verification) registered on an `APP_ENV`-only production deploy, and silently disabled the Google OAuth approved-school-domain allowlist entirely. Also `index.ts` (missing both `VERCEL_ENV` and `APP_ENV` — leaked Node stack traces in every API error response and allowed any `http://localhost:*` Origin through CORS with `credentials: true`), `services/email.ts` (`VERCEL_ENV`-only, gated only a startup diagnostic — lower severity), and `middleware/rateLimit.ts` (already correct, just duplicated). All fixed (commit `42369b1`) by switching to the canonical `isProdLike()`/`isPubliclyDeployed()`. Strengthened the architecture test with a second, name-independent check (raw pattern scan) so this exact evasion can't recur, plus 3 new tests in `previewDeploymentGatingArchitecture.test.ts` for the `/dev-signin` route and domain allowlist specifically. |
...
| §15 Client/server auth consistency | Single API client everywhere; no ad hoc `fetch` + manual token reads; HttpOnly cookie migration | ✅ Fixed the 3 concrete bugs the goal names almost verbatim (commit `d585b5b`): `SchoolBilling.tsx` read the wrong localStorage key entirely (`"token"` instead of `"goodhours_token"`) so procurement-document downloads sent `Authorization: Bearer null` on every click — completely non-functional, now fixed via the existing `api.download()` helper. `CreateOpportunity.tsx`'s edit path redirected immediately regardless of write success (`keepalive` fetch + empty `.catch()`) — now awaits `api.put()` and only redirects on confirmed success. Full repo grep found no other ad hoc `fetch`/raw-localStorage-token patterns outside of legitimately-public unauthenticated flows (email verification, cancellation links). **HttpOnly cookie migration — done this session** (commits `f751359` server, `32d79b8` client), scoped live with explicit product decisions on rollout (dual-mode: server accepts cookie or Bearer, no existing test/client breaks) and session UX (cookie Max-Age mirrors the remember-me toggle; deleted the ~85-line cross-tab localStorage token-sync mechanism, now redundant since cookies sync natively across tabs). Server sets an HttpOnly/SameSite=Lax cookie at all 9 token-issuance sites (login, password change, impersonation, Google OAuth, 5 invitation-accept flows); `authenticate()` and the rate limiter's per-user bucketing both check the cookie first, falling back to the Authorization header. Added `POST /api/auth/logout` (clears the cookie server-side — client JS can't clear an HttpOnly cookie) and `POST /api/auth/session-pref` (re-issues the cookie with the requested persistence). Client (`lib/authSession.ts`, `lib/api.ts`, `AuthProvider.tsx`) no longer stores the JWT anywhere JS-readable or attaches a Bearer header at all — this closes the actual XSS-token-theft surface the migration exists to close. `AuthProvider` now always confirms the session via `GET /auth/me` on mount (a cached user is still rendered optimistically to avoid a loading flash, but presence of that cache is no longer authoritative — correctly weaker but strictly *more* correct than the old token-presence check, which meant clearing localStorage alone was equivalent to logging out). Verified end-to-end against the real dev server+client (no browser extension available in this environment, so verified via `curl` with a real cookie jar rather than skipping live verification): login sets the expected cookie attributes, cookie-only requests authenticate with zero Authorization header, logout clears the cookie and subsequent requests correctly 401, session-pref re-issues without Max-Age when switching to session-only. 8 new server tests (`authCookieMigration.test.ts`); full server+client `tsc` and the 372-test server suite pass. **While scoping this, found and fixed 4 more independent instances of the earlier `isProdLike()`-duplication bug** (commit `42369b1`) in files this work touched — see the entry above, most severe being a fully-unauthenticated Google OAuth dev-signin bypass reachable on an `APP_ENV`-only production deploy. |
