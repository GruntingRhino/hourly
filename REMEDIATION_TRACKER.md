...
Only `ServiceSession.status`/`.verificationStatus` remains — the last core/heavily-cross-referenced model from the original sweep, where an initial broad grep matched many ambiguous string-literal call sites across unrelated models. Needs the same per-field verification each of these 22 got (now demonstrated at scale by the `User.status`, `School.ownershipStatus`, `Beneficiary`, and `BeneficiarySignup` conversions) — not yet started. §17.2 (float→Decimal hours) not attempted — larger blast radius, touches every hours calculation site. §17.3 (constraints): only the `ServiceSession` VERIFIED/checkOutTime constraint added; no broader audit. |
