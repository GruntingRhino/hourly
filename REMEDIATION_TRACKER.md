...
| `User.isTestAccount` (schema field + doc comment: "hidden from all lists") is written by both LMS integration services (Canvas/Google Classroom sync test-account creation) but was never read/filtered anywhere — QA/Playwright fixture students would appear mixed into real staff-facing rosters, reports, and messaging audiences. Found via the same dead-field sweep. | 🟡 | **Fixed at the highest-leverage point** (commit `58876ec`): added `isTestAccount: false` to `buildCohortScopedStudentWhere()` (`lib/cohortAccess.ts`), the central cohort-scoping helper `routes/{cohorts,reports,messages,selfSubmissions,schools,sessions}.ts` all already build their staff-facing student queries from — the same canonical mechanism established by the Finding #5 fix. **Known remaining gap**: ~13 other `role: "STUDENT"` query sites (mostly in `schools.ts`) construct their `where` clauses independently of this shared helper (bespoke category-cap-warning logic, ad hoc cohort filters, etc.) and still don't exclude test accounts — each needs individual review to fix safely; not rushed in this round. |
