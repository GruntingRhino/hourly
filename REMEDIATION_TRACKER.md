...
**Last updated:** 2026-08-04, commit `0ca486a`.
...
| §8 Canonical participation state machine | Explicit `WAITLISTED→CONFIRMED→ATTENDED/NO_SHOW→...` lifecycle enforced centrally; batched attendance with audit | 🟡 `statusTransitions.test.ts` covers the `PENDING_CHECKIN→CHECKED_IN→CHECKED_OUT→VERIFIED/REJECTED` machine for legacy sessions. **Fixed this session (commit `0ca486a`):** `POST /signups/:signupId/approve` allowed a `NO_SHOW` signup straight through to approval, silently resetting status to `CONFIRMED` and granting full hours with no override, no reason, no distinct audit trail — exactly §8.1's named-prohibited invalid state (`attendance = NO_SHOW, approvedHours > 0`), and the exact item security_findings.md's original P0 containment list named as unresolved. Now requires `overrideNoShow: true` + a reason, audited as a distinct `NO_SHOW_OVERRIDE_APPROVED` action. **Still open:** `BeneficiarySignup.status` ("NO_SHOW") and `.attendance` ("ATTENDED"/"NO_SHOW", set by a separate unbounded-`Promise.all` batch endpoint) are two independent fields that can drift out of sync — a real architectural inconsistency, not unified into one state machine. Beneficiary-signup states (`WAITLISTED`/`NO_SHOW`/etc.) remain a separate lifecycle from the legacy session machine, not consolidated |
...
- `cd server && npm test` → **283 pass / 0 fail / 1 skipped** (284 total). The 1 skip is infra-conditional (`RATE_LIMIT_TEST_DATABASE_URL` not set in this run) — verified separately against a real disposable Postgres DB with 0 skipped/0 failed
