    ...
    <main className="min-h-screen flex items-center justify-center bg-[var(--bg)] px-6 py-12">
      <section className="w-full max-w-xl rounded-2xl border border-[var(--border)] bg-[var(--card)] p-8 shadow-sm">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <button className="rounded-lg bg-[var(--accent)] px-4 py-2 font-medium text-white disabled:cursor-not-allowed disabled:opacity-50" onClick={() => void resend()} disabled={sending || cooldownUntil > now}>
            {sending ? "Sending…" : cooldownUntil > now ? "Email sent — resend in 15 minutes" : "Send approval email"}
          </button>
          <button className="rounded-lg border border-[var(--border)] px-4 py-2 font-medium text-[var(--text)]" onClick={() => void refreshUser()}>
            Check approval status
          </button>
          <button className="rounded-lg border border-[var(--border)] px-4 py-2 font-medium text-[var(--text)]" onClick={logout}>
            Sign out
          </button>
        </div>
      </section>
